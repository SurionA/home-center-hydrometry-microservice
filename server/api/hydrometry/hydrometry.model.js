 ...
 */
HydrometrySchema.virtual('hydrometry').get(function() {
    return {
        id: this._id,
        room: this.room,
    };
 ...
 */
HydrometrySchema.path('room').validate(room => room.length, 'Room cannot be empty');

/**
 * @description Every update set new updatedAt date
 */
HydrometrySchema.post('update', function() {
    this.update(
        {},
        {
            $set: {
